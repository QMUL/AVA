...
class YOLOLayer(nn.Module):
    def __init__(self, anchors, nC, nID, nE, img_size, yolo_layer, device):
    ...
    def forward(self, p_cat,  img_size, targets=None, classifier=None, test_emb=False):
        ...
        if targets is not None:
            if test_emb:
                tconf, tbox, tids = build_targets_max(targets, self.anchor_vec.to(self.device), self.nA, self.nC, nGh, nGw, self.device)
            else:
                tconf, tbox, tids = build_targets_thres(targets, self.anchor_vec.to(self.device), self.nA, self.nC, nGh, nGw, self.device)
...
class Darknet(nn.Module):
    ...
    def __init__(self, cfg_dict, device, nID=0, test_emb=False):
        ...
        self.hyperparams, self.module_list = create_modules(self.module_defs, self.device)
