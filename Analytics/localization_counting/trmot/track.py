...
def eval_seq(opt, dataloader, data_type, result_filename, countFileName, save_dir=None, show_image=True, frame_rate=30):
    ...
    for path, img, img0 in tqdm(dataloader):
        ...
        if opt.AVA:
            ...
            for id, box in zip(online_ids, online_tlwhs):
                txt += ',{:.1f},{:.1f},{:.1f},{:.1f},-2,-2,-2,-2,{:d},-2,-2'.format(box[0]*ratio_x, box[1]*ratio_y, (box[0]+box[2])*ratio_x, (box[1]+box[3])*ratio_y, id)
            txt += '\n'
            countFile.write(txt)
            countFile.close()   
        ...
        if opt.video_path==0:
            for id, box in zip(online_ids, online_tlwhs):
                cv2.rectangle(img0, (int(box[0]*ratio_x), int(box[1]*ratio_y)), (int((box[0]+box[2])*ratio_x), int((box[1]+box[3])*ratio_y)), (0, 255, 0), 2)
...
def main(opt, data_root='/data/MOT16/train', det_root=None, seqs=('MOT16-05',), exp_name='demo', 
    ...
    for seq in seqs:
        ...
        nf, ta, tc = eval_seq(opt, dataloader, data_type, result_filename, outPath,
