...
def build_targets_max(target, anchor_wh, nA, nC, nGh, nGw, device):
    """
...
def build_targets_thres(target, anchor_wh, nA, nC, nGh, nGw, device):
    ID_THRESH = 0.5
    FG_THRESH = 0.5
    BG_THRESH = 0.4
    nB = len(target)  # number of images in batch
...
def generate_anchor(nGh, nGw, anchor_wh, device):
    nA = len(anchor_wh)
...
def decode_delta_map(delta_map, anchors, device):
    '''
