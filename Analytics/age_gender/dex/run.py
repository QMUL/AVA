...
if args.AVA:
	if args.ov:
		outPath = '../../output/dex_ov'
	elif args.cpu:
		outPath = '../../output/dex_cpu'
	else:
		outPath = '../../output/dex_gpu'
...
#detections_path = '../../../people_counting/output/{}/retinaface_{}/{}.txt'.format(args.system,'gpu' if device=='cuda' else 'ov', video_name)
detections = estimationReader(args.localization_path, bodypart='face')


# setup model
api = api(device, args.ov)
...
for fr in tqdm(range(num_images)):
	...
	if len(bboxes):
		faces = np.empty((len(bboxes), 3, args.img_size, args.img_size))
		for i, box in enumerate(bboxes):
			...
			faces[i] = np.transpose(cv2.resize(frame[yw1:yw2 + 1, xw1:xw2 + 1], (args.img_size, args.img_size)), (2, 0, 1))
		faces = torch.from_numpy(faces).type('torch.FloatTensor')

		ages, genders = api.estimate(faces)		
